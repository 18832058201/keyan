                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            ...
                            <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toLengthWaysProjectListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                            <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toLengthWaysProjectListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                            <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toLengthWaysProjectListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        ...
                        <div class="widget-content nopadding">
                            ...
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tr>
                                    ...
                                    <c:if test="${auditType eq 1}">
                                    <th style="text-align: center;width: 3%;">操作</th>
                                    </c:if>
                                </tr>
                                </thead>
                                <tbody class="table">
                                ...
                                <tr class="gradeX">
                                    <td><input type="checkbox" name="auditObjectId" value="${project.projectId}" /></td>
                                    <td><a href="<%=request.getContextPath()%>/audit/viewLengthWaysProjectInfo.action?projectId=${project.projectId}&auditType=${auditType}">${project.projectName}</a></td>
                                    <td>${project.subjectTypeName}</td>
                                    <td>${project.projectNumber}</td>
                                    <td>${project.leader}</td>
                                    <td>${project.endStatusName}</td>
                                    <td>${project.auditStatusName}</td>
                                    <td>${ds.format(project.approveDate)}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${!empty project.attachment}">
                                                <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${project.attachment}">下载</a>
                                            </c:when>
                                            <c:otherwise>
                                                无
                                            </c:otherwise>
                                    ...
                                    <c:if test="${auditType eq 1}">
                                    <td>
                                        <a href="<%=request.getContextPath()%>/project/toEditLengthWaysProject.action?editType=audit&projectId=${project.projectId}">编辑</a>
                                    </td>
                                     </c:if>
                                </tr>
                                </c:forEach>
...
<script>
    ...
    function agree() {
        ...
        if(isChecked()){
            ...
            if(auditOpinion){  //确定审核操作
                ...
                $("#subForm").attr("action","<%=request.getContextPath()%>/audit/submitLengthWaysProjectAudit.action?operation=agree");   //修改提交路径
                $("#subForm").submit();                                                //提交表单
            }else {
                alert("提示：您已取消审核操作");
            }
    ...
    function refuse() {
        ...
        if(isChecked()){
            ...
            if(auditOpinion){  //确定审核操作
                ...
                $("#subForm").attr("action","<%=request.getContextPath()%>/audit/submitLengthWaysProjectAudit.action?operation=refuse");   //修改提交路径
                $("#subForm").submit();                                                //提交表单
            }else {
                alert("提示：您已取消审核操作");
            }
    ...
    function revoke() {
        //至少勾选了一项才能提交审核
        if(isChecked()){
            if(confirm("提示：撤销审核后项目将重新进入待审核状态，继续撤销吗？")){
                $("#subForm").attr("action","<%=request.getContextPath()%>/audit/submitLengthWaysProjectAudit.action?operation=revoke");   //修改提交路径
                $("#subForm").submit();
            }
            else {
                alert("提示：您已取消审核操作");
