...
<br>
&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:void(0)" onclick="submitProject()" class="button button-primary button-rounded button-small">提交</a>&nbsp;&nbsp;
<a href="javascript:void(0)" onclick="saveProject()" class="button button-primary button-rounded button-small">保存</a>
<br>
<br>
<%--按钮end--%>
<h1 class="title-h1">新增纵向项目</h1>
<br>
...
<form action="" method="POST" id="projectForm" enctype="multipart/form-data" >
    ...
    <table style="table-layout:fixed;">
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="text" name="projectName" value="${projectInfo.projectName}" required style="width: 500px;" onblur="nameIsExisted(this.value,'<%=request.getContextPath()%>/project/addIsExistedLengthWaysProject.action')">
        ...
        <tr>
            ...
            <td colspan="3" class="ibg">
                <input type="text" name="projectNumber" value="${projectInfo.projectNumber}" required style="width: 200px;">
                <label class="required">*</label>
            </td>
            <td class="right left">项目性质：</td>
            <td colspan="4" class="ibg">&nbsp;&nbsp;纵向项目
                <%--<label class="required">*</label>--%>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="4" class="ibg">
                <input type="text" class="Wdate" readonly="readonly" onclick="WdatePicker()"
                       style="width: 200px;" name="approveDate" required value="${ds.format(projectInfo.approveDate)}"/>
                <label class="required">*</label>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="4" class="ibg">
                <select name="organizationFormId" class="select-min" required>
                    <option selected="selected" value="">--请选择--</option>
                    <c:forEach items="${organizationFormList}" var="organizationForm">
                        <option value="${organizationForm.organizationFormId}">${organizationForm.organizationFormName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
        ...
        <tr style="display: none">
            ...
            <td colspan="8" class="ibg">
                <select name="economicsTargetId" class="select-min" required>
                    <option selected="selected" value="">--请选择--</option>
                    <c:forEach items="${economicsTargetList}" var="economicsTarget">
                        <option value="${economicsTarget.economicsTargetId}">${economicsTarget.economicsTargetName}</option>
                    </c:forEach>
                </select>
        ...
        <tr style="display: none">
            ...
            <td colspan="8" class="ibg">
                <select name="cooperationTypeId" class="select-min" required>
                    <option selected="selected" value="">--请选择--</option>
                    <c:forEach items="${cooperationTypeList}" var="cooperationType">
                        <option value="${cooperationType.cooperationTypeId}">${cooperationType.cooperationTypeName}</option>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="text" name="cooperationUnit" style="width: 500px;" required value="${projectInfo.cooperationUnit}">
            ...
            <td colspan="8" class="ibg">
                <select name="economicsIndustry1Id" required class="select-min" onchange="economicsIndustry1Change(this.value)">
                    ...
                    <c:forEach items="${economicsIndustry1List}" var="economicsIndustry1">
                        <option value="${economicsIndustry1.economicsIndustry1Id}">${economicsIndustry1.economicsIndustry1Name}</option>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                ...
                <input type="text" name="resultForm" required  style="width: 300px;" value="${projectInfo.resultForm}">
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="number" name="approveMoney" required value="${projectInfo.approveMoney}">
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <label>是<input type="radio" name="inspect" value="1"></label>
                <label>否<input type="radio" name="inspect" value="0" checked = "checked"></label>
            </td>
        </tr>
        <tr>
            <td class="right">项目所属类型：</td>
            <td colspan="3" class="ibg">
                <select name="projectDomainId" class="select-min" required>
                    <option selected="selected" value="">--请选择--</option>
                    <c:forEach items="${projectDomainList}" var="projectDomain">
                        <option value="${projectDomain.projectDomainId}">${projectDomain.projectDomainName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="4" class="ibg">
                <select name="projectGradeId" class="select-min" required>
                    <option selected="selected" value="">--请选择--</option>
                    <c:forEach items="${projectGradeList}" var="projectGrade">
                        <option value="${projectGrade.projectGradeId}">${projectGrade.projectGradeName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <label>是<input type="radio" name="important" value="1"></label>
                <label>否<input type="radio" name="important" value="0" checked="checked"></label>
                <label class="required">*</label>
            </td>
        </tr>
        <tr>
    ...
    <table style="table-layout:fixed;">
        ...
        <%--<tr>--%>
            <%--<td class="right">其他经费：</td>--%>
            <%--<td colspan="3" class="ibg">--%>
                <%--<input type="number" name="otherTotal" required>--%>
                <%--<label class="required">*（单位：元）</label>--%>
            <%--</td>--%>
        <%--</tr>--%>
        <tr>
...
<script>
/*
 改变学科门类时更改与其关联的项    由于c标签中无法获取js中的参数值，此方法不适用
 */
<%--function subjectTypeChange(subjectTypeId) {--%>
    <%--var subjectFirstId = $("select[name='subjectFirstId']"); //获取到学科门类选择框--%>
    <%--var subjectFirstIdHtml =  '<option value="-1">--请选择--</option>'; //初始化下拉框内容--%>
    <%--&lt;%&ndash;for(${initParam};i<${subjectFirstList.size()};i++){  //遍历一级学科集合&ndash;%&gt;--%>
    <%--&lt;%&ndash;if(subjectFirstId===${subjectFirstList.get(i).subjectTypeId}){&ndash;%&gt;--%>
    ...
    <%--<c:if test="${true}">--%>
    <%--subjectFirstIdHtml+='<option value="${subjectFirst.subjectFirstId}">${subjectFirst.subjectFirstName}</option>';--%>
    <%--</c:if>--%>
    <%--</c:forEach>--%>
    <%--subjectFirstId.html(subjectFirstIdHtml);--%>
...
function subjectTypeChange(subjectTypeId) {
    ...
    //显示/隐藏相关标签，对显示的标签启用必填验证，隐藏的取消必填验证
...
function saveProject() {
    ...
    if (confirm("提示：确认保存吗？")) {
        //修改提交地址为 save
        $("#projectForm").attr("action", "<%=request.getContextPath()%>/project/insertLengthwaysProject.action?submitType=save");
...
function submitProject() {
    ...
    if (confirm("提示：确认提交吗？")) {
        //修改提交地址为 submit
        $("#projectForm").attr("action", "<%=request.getContextPath()%>/project/insertLengthwaysProject.action?submitType=submit");
        $("#subForm").click(); //触发按钮提交事件
    }
}
