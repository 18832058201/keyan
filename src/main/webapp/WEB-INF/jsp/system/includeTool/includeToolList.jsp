...
<body>
<div id="content">
    <form action="<%=request.getContextPath()%>/insertIncludeTool.action" class="form-horizontal" method="post"
          id="insertForm">
                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            <label style="float:left;padding-top: 7px;padding-left: 20px;">收录工具列表，共有&nbsp;&nbsp;${includeToolList.size()}&nbsp;&nbsp;条数据</label>
                        </div>
                        <div class="widget-content nopadding">
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tr>
                                    ...
                                    <th style="text-align: center;width: 30%;">收录工具名称</th>
                                ...
                                <c:forEach items="${includeToolList}" var="includeTool">
                                    <tr class="gradeX">
                                        <td>${includeTool.includeToolId}</td>
                                        <td>${includeTool.includeToolName}</td>
                                        <td>${includeTool.score}</td>
                                        <td>${includeTool.isEffective eq 1 ? "有效":"无效"}</td>
                                        <td>
                                            <a href="javascript:void(0)" onclick="toEdit(this)">编辑</a>
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${includeTool.includeToolId},0)" ${includeTool.isEffective eq 1 ? 'style=""':'style="display:none"'}>置为无效</a>
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${includeTool.includeToolId},1)" ${includeTool.isEffective eq 0 ? 'style=""':'style="display:none"'}>设为有效</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                                ...
                                <tr class="gradeX">
                                    <td></td>
                                    <td><input type="text" name="includeToolName" onblur="nameIsExisted(this.value,'/includeToolNameIsExisted.action')" style="width: 80%;height:30px"/></td>
                                    ...
                                    <td><a href="javascript:void(0)" onclick="insertIncludeTool()">新增</a></td>
                                </tr>
                                </tfoot>
                            </table>
                        </div>
...
<script>
    ...
    function toEdit(obj){
        ...
        if($(obj).html()==="编辑"){
            ...
            td2.html('<input type="text" name="includeToolName" style="width: 80%;height:30px" value="'+ td2.html() +'">');
            //获取到第三列元素
            var td3 = tr.find("td").eq(2);
            td3.html('<input type="text" name="score" style="width: 80%;height:30px" value="'+ td3.html() +'">');
        }else{
            var includeToolId = tr.find("td").eq(0).html(); //获取收录工具id
            var includeToolName = tr.find("td").eq(1).find("input").val();  //获取收录工具
            var score = tr.find("td").eq(2).find("input").eq(0).val();   //获取考核得分
            location.href ="updateIncludeTool.action?includeToolName="+ includeToolName +"&score="+ score +"&includeToolId="+ includeToolId +"";
        }
    }

    //更新是否有效状态
    function updateIsEffective(includeToolId,isEffective) {
        if (confirm("确认更新吗？")) {
            location.href = "updateIncludeTool.action?includeToolId=" + includeToolId + "&isEffective="+ isEffective +"";
        }
    }

    //插入数据
    function insertIncludeTool() {
        if (confirm("确认新增吗？")) {
            $("#insertForm").submit();
        }
    }
