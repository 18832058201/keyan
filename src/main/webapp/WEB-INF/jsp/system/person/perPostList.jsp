...
<body>
<div id="content">
    <form action="<%=request.getContextPath()%>/insertPerPost.action" class="form-horizontal" method="post"
          id="insertForm">
                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            <label style="float:left;padding-top: 7px;padding-left: 20px;">绩效列表，共有&nbsp;&nbsp;${perPostCount}&nbsp;&nbsp;条数据</label>
                        </div>
                        <div class="widget-content nopadding">
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tr>
                                    ...
                                    <th style="text-align: center;width: 30%;">绩效类别</th>
                                    <th style="text-align: center;width: 20%;">绩效等级</th>
                                ...
                                <tbody class="table">
                                <%--数据库中查询的数据--%>
                                <c:forEach items="${perPostLevelList}" var="perPostLevel">
                                    <tr class="gradeX">
                                        <td>${perPostLevel.postLevelId}</td>
                                        <td>
                                            <c:forEach items="${perPostTypeList}" var="perPostType">
                                                <c:if test="${perPostType.postTypeId eq perPostLevel.postTypeId}">
                                                    ${perPostType.postTypeName}
                                                </c:if>
                                            </c:forEach>
                                            <input type="hidden" value="${ perPostLevel.postTypeId}"/>
                                        </td>
                                        <td>${perPostLevel.postLevelName}</td>
                                        <td>${perPostLevel.rate}</td>
                                        <td>
...
<script>
    ...
    function toEdit(obj){
        ...
        if($(obj).html()==="编辑"){
            ...
            var postLevelId = tr.find("td").eq(0).html(); //绩效等级id
            var rate = tr.find("td").eq(3).find("input").eq(0).val();   //获取倍率
            location.href ="/updatePerPostLevel.action?postLevelId="+postLevelId+"&rate="+rate+"";
        }
    }
</script>
<%--返回异常信息--%>
