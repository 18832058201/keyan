...
<body>
<div id="content">
    <form action="<%=request.getContextPath()%>/insertProProjectGrade.action" class="form-horizontal" method="post"
          id="insertForm">
                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            <label style="float:left;padding-top: 7px;padding-left: 20px;">项目等级列表，共有&nbsp;&nbsp;${ProProjectGradeCount}&nbsp;&nbsp;条数据</label>
                        </div>
                        <div class="widget-content nopadding">
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tr>
                                    ...
                                    <th style="text-align: center;width: 30%;">项目等级</th>
                                ...
                                <tbody class="table">
                                <%--数据库中查询的数据--%>
                                <c:forEach items="${ProProjectGradeList}" var="proProjectGrade">
                                    <tr class="gradeX">
                                        <td>${proProjectGrade.projectGradeId}</td>
                                        <td>${proProjectGrade.projectGradeName}</td>
                                        <td>${proProjectGrade.isEffective eq 1 ? "有效":"无效"}</td>
                                        <td>
                                            ...
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${proProjectGrade.projectGradeId},0)" ${proProjectGrade.isEffective eq 1 ? 'style=""':'style="display:none"'}>置为无效</a>
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${proProjectGrade.projectGradeId},1)" ${proProjectGrade.isEffective eq 0 ? 'style=""':'style="display:none"'}>设为有效</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                                ...
                                <tr class="gradeX">
                                    <td></td>
                                    <td><input type="text" name="projectGradeName" style="width: 80%;height:30px"/></td>
                                    <td>
...
<script>
    ...
    //更新是否有效状态
    function updateIsEffective(projectGradeId,isEffective) {
        if (confirm("确认更新吗？")) {
            location.href = "updateProProjectGrade.action?projectGradeId=" + projectGradeId + "&isEffective="+isEffective+"";
        }
    }
    ...
    function toEdit(obj){
        ...
        if($(obj).html()==="编辑"){
            ...
            td2.html('<input type="text" name="projectGradeName" style="width: 80%;height:30px" value="'+td2.html()+'">');

        }else{
            var projectGradeId = tr.find("td").eq(0).html(); //获取获奖类别id
            var projectGradeName = tr.find("td").eq(1).find("input").val();  //获取获奖类别名称
            location.href ="/updateProProjectGrade.action?projectGradeName="+projectGradeName+"&projectGradeId="+projectGradeId+"";
        }
    }
</script>
