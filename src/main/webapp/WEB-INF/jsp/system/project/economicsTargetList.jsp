...
<body>
<div id="content">
    <form action="<%=request.getContextPath()%>/insertProEconomicsTarget.action" class="form-horizontal" method="post"
          id="insertForm">
                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            <label style="float:left;padding-top: 7px;padding-left: 20px;">社会经济目标列表，共有&nbsp;&nbsp;${ProEconomicsTargetCount}&nbsp;&nbsp;条数据</label>
                        </div>
                        <div class="widget-content nopadding">
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tr>
                                    ...
                                    <th style="text-align: center;width: 30%;">社会经济目标名称</th>
                                ...
                                <tbody class="table">
                                <%--数据库中查询的数据--%>
                                <c:forEach items="${proEconomicsTargetList}" var="proEconomicsTarget">
                                    <tr class="gradeX">
                                        <td>${proEconomicsTarget.economicsTargetId}</td>
                                        <td>${proEconomicsTarget.economicsTargetName}</td>
                                        <td>${proEconomicsTarget.isEffective eq 1 ? "有效":"无效"}</td>
                                        <td>
                                            ...
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${proEconomicsTarget.economicsTargetId},0)" ${proEconomicsTarget.isEffective eq 1 ? 'style=""':'style="display:none"'}>置为无效</a>
                                            <a href="javascript:void(0)" onclick="updateIsEffective(${proEconomicsTarget.economicsTargetId},1)" ${proEconomicsTarget.isEffective eq 0 ? 'style=""':'style="display:none"'}>设为有效</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                                ...
                                <tr class="gradeX">
                                    <td></td>
                                    <td><input type="text" name="economicsTargetName" style="width: 80%;height:30px"/></td>
                                    <td>
                                        <select name="isEffective" style="width: 80%;height:30px">
                                            <option value="1">有效</option>
                                            <option value="0">无效</option>
                                        </select></td>
                                    <td><a href="javascript:void(0)" onclick="insertEconomicsTargetName()">新增</a></td>
                                </tr>
                                </tfoot>
                            </table>
                        </div>
...
<script>
    //插入数据
    function insertEconomicsTargetName() {
        if (confirm("确认新增吗？")) {
            $("#insertForm").submit();
        }
    }
    //更新是否有效状态
    function updateIsEffective(economicsTargetId,isEffective) {
        if (confirm("确认更新吗？")) {
            location.href = "updateProEconomicsTarget.action?economicsTargetId=" + economicsTargetId + "&isEffective="+isEffective+"";
        }
    }
    ...
    function toEdit(obj){
        ...
        if($(obj).html()==="编辑"){
            ...
            td2.html('<input type="text" name="economicsTargetName" style="width: 80%;height:30px" value="'+td2.html()+'">');

        }else{
            var economicsTargetId = tr.find("td").eq(0).html(); //获取获奖类别id
            var economicsTargetName = tr.find("td").eq(1).find("input").val();  //获取获奖类别名称
            location.href ="/updateProEconomicsTarget.action?economicsTargetName="+economicsTargetName+"&economicsTargetId="+economicsTargetId+"";
        }
    }
</script>
