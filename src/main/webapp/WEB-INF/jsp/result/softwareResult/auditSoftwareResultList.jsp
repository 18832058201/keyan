            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                        <label style="float:left;padding-top: 7px;padding-left: 20px;">软件著作列表，共有&nbsp;&nbsp;${softwareRltList.size()}&nbsp;&nbsp;条数据</label>
                        <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toSoftwareResultListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toSoftwareResultListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toSoftwareResultListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                    ...
                    <div class="widget-content nopadding">
                        ...
                        <table class="table table-bordered data-table" style="table-layout:fixed;">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;width: 10%;">证书日期</th>
                            ...
                            <tbody class="table">
                            <c:forEach items="${softwareRltList}" var="software">
                            <tr class="gradeX">
                                <td><input type="checkbox" name="auditObjectId" value="${software.softwareId}"></td>
                                <td><a href="<%=request.getContextPath()%>/audit/viewSoftwareResultInfo.action?id=${software.softwareId}&auditType=${auditType}">${software.softwareName}</a></td>
                                <td>${software.subjectTypeName}</td>
                                <td>${software.registerNumber}</td>
                                <td>${software.certificateNumber}</td>
                                <td>${software.owner}</td>
                                <td>${software.personList}</td>
                                <td>${ds.format(software.certificateDate)}</td>
                                <td>${software.auditStatusName}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${!empty software.attachment}">
                                            <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${software.attachment}">下载</a>
                                        </c:when>
                                        <c:otherwise>
                                            无
                                        </c:otherwise>
                                ...
                                <c:if test="${auditType eq 1}">
                                    <td>
                                        <a href="<%=request.getContextPath()%>/result/softwareResult/toEditSoftwareResult.action?editType=audit&softwareId=${software.softwareId}">编辑</a>
                                    </td>
                                </c:if>
                            </tr>
                            </c:forEach>
