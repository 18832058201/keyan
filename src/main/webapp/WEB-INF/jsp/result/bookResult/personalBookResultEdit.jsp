...
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:void(0)" onclick="submitBookResult()" class="button button-primary button-rounded button-small">提交</a>&nbsp;&nbsp;
...
<%--表单start--%>
<form action="" method="POST" id="updateBookResultForm" enctype="multipart/form-data">
    <%--设置隐藏提交按钮--%>
    ...
    <table style="table-layout:fixed;">
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="text" style="width: 500px;" name="bookName" value="${bookResultVO.bookName}" required
                       onblur="nameIsExisted(this.value,'/result/bookResult/bookNameIsExisted.action?bookId=${bookResultVO.bookId}')">
                <input type="hidden" value="${bookResultVO.bookId}" name="bookId">
            ...
            <td colspan="3" class="ibg">
                <select name="subjectTypeId" class="select-min" onchange="subjectTypeChange(this.value)" required>
                    ...
                    <c:forEach items="${subjectTypeList}" var="subjectType">
                        <option value="${subjectType.subjectTypeId}" ${subjectType.subjectTypeId eq bookResultVO.subjectTypeId ? "selected='selected'":""}>${subjectType.subjectTypeName}</option>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td colspan="3" class="ibg">
                <select name="bookTypeId" class="select-min" required>
                    ...
                    <c:forEach items="${bookTypeList}" var="bookType">
                        <option value="${bookType.bookTypeId}" ${bookType.bookTypeId eq bookResultVO.bookTypeId ? "selected='selected'":""}>${bookType.bookTypeName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="4" class="ibg">
                <input type="text" style="width: 200px;" name="isbn" value="${bookResultVO.isbn}" required>
                <label class="required">*</label>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="3" class="ibg">
                <input type="text" style="width: 200px;" name="totalNum" value="${bookResultVO.totalNum}" required>
                <label class="required">*</label>
            </td>
            <td class="right left">撰写总字数：</td>
            <td colspan="4" class="ibg">
                <input type="text" style="width: 200px;" name="writeNum" value="${bookResultVO.writeNum}" required>
                <label class="required">*</label>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="3" class="ibg">
                <input type="text" style="width: 200px;" name="publishUnit" value="${bookResultVO.publishUnit}" required>
                <label class="required">*</label>
            </td>
            <td class="right left">出版地：</td>
            <td colspan="4" class="ibg">
                <input type="text" style="width: 200px;" name="publishPlace" value="${bookResultVO.publishPlace}" required>
                <label class="required">*</label>
            </td>
        ...
        <tr>
            ...
            <td colspan="3" class="ibg">
                <input type="text" class="Wdate" readonly="readonly" onclick="WdatePicker()"
                       value="${ds.format(bookResultVO.publishDate)}"
                       style="width: 200px;" name="publishDate" required/>
                <label class="required">*</label>
            </td>
            <td class="right left">出版社类型：</td>
            <td colspan="4" class="ibg">
                <select name="bookPressTypeId" class="select-min" required>
                    <option value="">--请选择--</option>
                    <c:forEach items="${bookPressTypeList}" var="pressType">
                        <option value="${pressType.pressTypeId}" ${pressType.pressTypeId eq bookResultVO.pressTypeId ? "selected='selected'":""}>${pressType.pressTypeName}</option>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                ...
                <label class="required">${empty bookResultVO.attachment ? "您还没有上传附件" : "您已上传过附件，如需重传请选择附件"}</label>
            </td>
        </tr>
        <tr>
            <td class="right">备注：</td>
            <td colspan="8" class="ibg">
                <textarea style="width:80%;height: 100px;resize: none;"
                          required>${bookResultVO.remark}</textarea>
...
<script>
    ...
    function saveBookResult() {
        ...
        if (confirm("确认保存该著作成果信息吗？")) {
            //修改提交地址为 save
            $("#updateBookResultForm").attr("action", "<%=request.getContextPath()%>/result/bookResult/updateBookResult.action?submitType=save");
    ...
    function submitBookResult() {
        ...
        if (confirm("确定提交该著作成果信息吗？")) {
            //修改提交地址为 submit
            $("#updateBookResultForm").attr("action", "<%=request.getContextPath()%>/result/bookResult/updateBookResult.action?submitType=submit");
            $("#subForm").click(); //触发按钮提交事件
        }
    }
</script>
