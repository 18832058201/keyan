            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toBookResultListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toBookResultListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toBookResultListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                    ...
                    <div class="widget-content nopadding">
                        ...
                        <table class="table table-bordered data-table" style="table-layout:fixed;">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;width: 10%;">出版单位</th>
                                <th style="text-align: center;width: 7%;">著作类型</th>
                                <th style="text-align: center;width: 8%;">出版地</th>
                            ...
                            <tr class="gradeX">
                                <td><input type="checkbox" name="auditObjectId" value="${book.bookId}"></td>
                                <td><a href="<%=request.getContextPath()%>/audit/viewBookResultInfo.action?id=${book.bookId}&auditType=${auditType}">${book.bookName}</a></td>
                                <td>${book.isbn}</td>
                                <td>${book.personList}</td>
                                <td>${book.publishUnit}</td>
                                <td>${book.bookTypeName}</td>
                                <td>${book.publishPlace}</td>
                                <td>${ds.format(book.publishDate)}</td>
                                <td>${book.auditStatusName}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${!empty book.attachment}">
                                            <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${book.attachment}">下载</a>
                                        </c:when>
                                        <c:otherwise>
                                            无
                                        </c:otherwise>
                                ...
                                <c:if test="${auditType eq 1}">
                                    <td>
                                        <a href="<%=request.getContextPath()%>/result/bookResult/toEditBookResult.action?editType=audit&bookId=${book.bookId}">编辑</a>
                                    </td>
                                </c:if>
                            </tr>
                            </c:forEach>
