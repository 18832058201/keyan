    ...
    <table style="table-layout:fixed;">
            ...
            <td colspan="3" class="ibg">
                <select name="subjectTypeId" class="select-min" onchange="subjectTypeChange(this.value)" required>
                    ...
                    <c:forEach items="${subjectTypeList}" var="subjectType">
                        <option value="${subjectType.subjectTypeId}" ${subjectType.subjectTypeId eq rewardResultVO.subjectTypeId ? "selected='selected'":""}>${subjectType.subjectTypeName}</option>
                    </c:forEach>
                </select>
            ...
            <td colspan="4" class="ibg">
                <select name="resultTypeId" class="select-min" required>
                    ...
                    <c:forEach items="${resultTypeList}" var="resultType">
                        <option value="${resultType.resultTypeId}" ${resultType.resultTypeId eq rewardResultVO.resultTypeId ? "selected='selected'":""}>${resultType.resultTypeName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="text" name="resultName" id="resultName" style="width: 500px;" value="${rewardResultVO.resultName}" required
                       onblur="nameIsExisted(this.value,'<%=request.getContextPath()%>/result/rewardResult/rewardNameIsExisted.action?rewardId=${rewardResultVO.rewardId}')">
                <input type="hidden" value="${rewardResultVO.rewardId}" name="rewardId">
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <select name="rewTypeId" class="select-min">
                    <option value="">--请选择--</option>
                    <c:forEach items="${rewTypeList}" var="rewType">
                        <option value="${rewType.rewTypeId}" ${rewType.rewTypeId eq rewardResultVO.rewTypeId ? "selected='selected'":""}>${rewType.rewTypeName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="4" class="ibg">
                <select name="subjectFirstId" class="select-min" required>
                    ...
                    <c:forEach items="${subjectFirstList}" var="subjectFirst">
                        <option value="${subjectFirst.subjectFirstId}" ${subjectFirst.subjectFirstId eq rewardResultVO.subjectFirstId ? "selected='selected'":""}>${subjectFirst.subjectFirstName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="8" class="ibg">
                <select name="rewClassId" class="select-min" required>
                    ...
                    <c:forEach items="${rewClassList}" var="rewClass">
                        <option value="${rewClass.rewClassId}" ${rewClass.rewClassId eq rewardResultVO.rewClassId?"selected='selected'":""}>${rewClass.rewClassName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="8" class="ibg">
                <select name="rewLevelId" class="select-min" onchange="rewLevelChange(this.value)" required>
                    ...
                    <c:forEach items="${rewLevelList}" var="rewLevel">
                        <option value="${rewLevel.rewLevelId}" ${rewLevel.rewLevelId eq rewardResultVO.rewLevelId?"selected='selected'":""}>${rewLevel.rewLevelName}</option>
                    </c:forEach>
                </select>
                <select name="finishWayId" id="finishWayId" class="select-min">
                    <option value="">--请选择完成方式--</option>
                    <c:forEach items="${finishWayList}" var="finishWay">
                        <option value="${finishWay.finishWayId}" ${finishWay.finishWayId eq rewardResultVO.finishWayId?"selected='selected'":""}>${finishWay.finishWayName}</option>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td colspan="3" class="ibg">
                <select name="rewGradeId" class="select-min" required>
                    ...
                    <c:forEach items="${rewGradeList}" var="rewGrade">
                        <option value="${rewGrade.rewGradeId}" ${rewGrade.rewGradeId eq rewardResultVO.rewGradeId ? "selected='selected'":""}>${rewGrade.rewGradeName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
            ...
            <td colspan="4" class="ibg">
                <input type="text" style="width: 200px;" name="rewCertificateNum"
                       value="${rewardResultVO.rewCertificateNum}" required>
                <label class="required">*</label>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="3" class="ibg">
                <input type="text" class="Wdate" readonly="readonly" onclick="WdatePicker()"
                       value="${ds.format(rewardResultVO.rewDate)}"
            ...
            <td colspan="4" class="ibg">
                <select name="signId" class="select-min" required>
                    ...
                    <c:forEach items="${signList}" var="sign">
                        <option value="${sign.signId}" ${sign.signId eq rewardResultVO.signId?"selected='selected'":""}>${sign.signName}</option>
                    </c:forEach>
                </select>
                <label class="required">*</label>
            </td>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <input type="text" name="rewUnit" style="width: 500px;" value="${rewardResultVO.rewUnit}" required>
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <textarea name="remark" style="width:80%;height: 100px;resize: none;"
                          required>${rewardResultVO.remark}</textarea>
                        ...
                        <c:forEach items="${personList}" var="person">
                            <tr>
                                ...
                                <td class="left align-center">
                                    <c:choose>
                                        <c:when test="${person.workName eq ds.getName(rewardResultVO.registrant)}"><label
...
<script>
    ...
    function saveIdentify() {
        ...
        if (confirm("提示：确认保存吗？")) {
            //修改提交地址为 save
            $("#rewardResultForm").attr("action", "<%=request.getContextPath()%>/audit/updateRewardResultAtAudit.action??submitType=onlySave");
    ...
    function saveAndAuditIdentify() {
        ...
        if(auditOpinion){  //确定审核操作
            ...
            $("#rewardResultForm").attr("action", "<%=request.getContextPath()%>/audit/updateRewardResultAtAudit.action?submitType=saveAndAudit&operation=agree");
            $("#subForm").click(); //触发按钮提交事件
        }else {
            alert("提示：您已取消审核操作");
        }
