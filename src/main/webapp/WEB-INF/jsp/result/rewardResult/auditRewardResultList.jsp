            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toRewardResultListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toRewardResultListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toRewardResultListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                    ...
                    <div class="widget-content nopadding">
                        ...
                        <table class="table table-bordered data-table" style="table-layout:fixed;">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;width: 15%;">获奖日期</th>
                            ...
                            <tbody class="table">
                            <c:forEach items="${rewardResultList}" var="reward">
                            <tr class="gradeX">
                                <td><input type="checkbox" name="auditObjectId" value="${reward.rewardId}"></td>
                                <td><a href="<%=request.getContextPath()%>/audit/viewRewardResultInfo.action?id=${reward.rewardId}&auditType=${auditType}">${reward.resultName}</a></td>
                                <td>${reward.rewCertificateNum}</td>
                                <td>${reward.subjectTypeName}</td>
                                <td>${reward.rewLevelName}</td>
                                <td>${reward.leader}</td>
                                <td>${reward.auditStatusName}</td>
                                <td>${ds.format(reward.rewDate)}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${!empty reward.attachment}">
                                            <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${reward.attachment}">下载</a>
                                        </c:when>
                                        <c:otherwise>
                                            无
                                        </c:otherwise>
                                ...
                                <c:if test="${auditType eq 1}">
                                    <td>
                                        <a href="<%=request.getContextPath()%>/result/rewardResult/toEditRewardResult.action?editType=audit&rewardId=${reward.rewardId}">编辑</a>
                                    </td>
                                </c:if>
                            </tr>
                            </c:forEach>
