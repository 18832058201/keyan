            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toPatentResultListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toPatentResultListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toPatentResultListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                    ...
                    <div class="widget-content nopadding">
                        ...
                        <table class="table table-bordered data-table" style="table-layout:fixed;">
                            ...
                            <tr class="gradeX">
                                <td><input type="checkbox" name="auditObjectId" value="${patent.patentId}"></td>
                                <td><a href="<%=request.getContextPath()%>/audit/viewPatentResultInfo.action?id=${patent.patentId}&auditType=${auditType}">${patent.patentName}</a></td>
                                <td>${patent.patentNumber}</td>
                                <td>${patent.patentee}</td>
                                <td>${patent.patTypeName}</td>
                                <td>${patent.leader}</td>
                                <td>${ds.format(patent.certificateDate)}</td>
                                <td>${patent.auditStatusName}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${!empty patent.attachment}">
                                            <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${patent.attachment}">下载</a>
                                        </c:when>
                                        <c:otherwise>
                                            无
                                        </c:otherwise>
                                ...
                                <c:if test="${auditType eq 1}">
                                <td>
                                    <a href="<%=request.getContextPath()%>/result/patentResult/toEditPatentResult.action?editType=audit&patentId=${patent.patentId}">编辑</a>
                                </td>
                                </c:if>
                            </tr>
                            </c:forEach>
