                ...
                <div class="span12">
                    <div class="widget-box">
                        <div class="widget-title" style="line-height: 36px;"><span class="icon"><i class="icon-th"></i></span>
                            ...
                            <label style="float:right;padding-top: 7px;padding-right: 30px;"><a href="<%=request.getContextPath()%>/audit/toIdentifyResultListAudit.action?auditType=2"><input type="radio" name="radio" ${auditType eq 2?'checked="checked"':""}>已审核</a></label>
                            <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toIdentifyResultListAudit.action?auditType=1"><input type="radio" name="radio" ${auditType eq 1?'checked="checked"':""}>待审核</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                            <label style="float:right;padding-top: 7px;"><a href="<%=request.getContextPath()%>/audit/toIdentifyResultListAudit.action?auditType=3"><input type="radio" name="radio" ${auditType eq 3?'checked="checked"':""}>全部</a>&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        ...
                        <div class="widget-content nopadding">
                            ...
                            <table class="table table-bordered data-table" style="table-layout:fixed;">
                                ...
                                <tbody class="table">
                                <c:forEach items="${identifyResultList}" var="identifyResult">
                                    <tr class="gradeX">
                                        <td><input type="checkbox" name="auditObjectId" value="${identifyResult.identifyId}"  ></td>
                                        <td><a href="<%=request.getContextPath()%>/audit/viewIdentifyResultInfo.action?id=${identifyResult.identifyId}&auditType=${auditType}">${identifyResult.projectName}</a> </td>
                                        <td>${identifyResult.projectSourceName}</td>
                                        <td>${identifyResult.formName}</td>
                                        <td>${identifyResult.idenLevelName}</td>
                                        <td>${identifyResult.idenCertificateNum}</td>
                                        <td>${identifyResult.registrant}</td>
                                        <td>${identifyResult.auditStatusName}</td>
                                        <td>${ds.format(identifyResult.identifyDate)}</td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${!empty identifyResult.attachment}">
                                                    <a href="<%=request.getContextPath()%>/file/fileDownload.action?fileName=${identifyResult.attachment}">下载</a>
                                                </c:when>
                                                <c:otherwise>
                                                    无
                                                </c:otherwise>
                                        ...
                                        <c:if test="${auditType eq 1}">
                                            <td>
                                                <a href="<%=request.getContextPath()%>/result/identifyResult/toEditIdentifyResult.action?editType=audit&identifyId=${identifyResult.identifyId}">编辑</a>
                                            </td>
                                        </c:if>
                                    </tr>
                                </c:forEach>
