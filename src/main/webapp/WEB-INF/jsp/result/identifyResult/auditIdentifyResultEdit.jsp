...
<form action="" method="POST" id="identifyResultForm" enctype="multipart/form-data">
    ...
    <table style="table-layout:fixed;">
        ...
        <tr>
            ...
            <td colspan="8" class="ibg">
                <textarea style="width:80%;height: 100px;resize: none;" name="remark" required>${identifyResultVO.remark}</textarea>
                        ...
                        <c:forEach items="${personList}" var="person">
                            <tr>
                                ...
                                <td class="left align-center" >
                                    <c:choose>
                                        <c:when test="${person.workName eq ds.getName(identifyResultVO.registrant)}"><label class="required">登记人无法删除</label></c:when>
...
<script>
    ...
    function saveIdentify() {
        ...
        if(confirm("提示：确定保存吗？")){
            //修改提交地址为 save
            $("#identifyResultForm").attr("action","<%=request.getContextPath()%>/audit/updateIdentifyResultAtAudit.action?submitType=onlySave");
    ...
    function saveAndAuditIdentify() {
        ...
        if(auditOpinion){  //确定审核操作
            ...
            $("#identifyResultForm").attr("action","<%=request.getContextPath()%>/audit/updateIdentifyResultAtAudit.action?submitType=saveAndAudit&operation=agree");   //修改提交路径
